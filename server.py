...
app.add_middleware(
    CORSMiddleware,
    # Change this to your frontend URL http://127.0.0.1:8000
...
async def search(search_query: str):
    ...
    image_data_list = []
    for idx, image_info in enumerate(results):
        # Assuming 'image_info' is a dictionary and it contains a key 'path' that has the file path
        if isinstance(image_info, dict) and 'path' in image_info:
            image_path = image_info['path']
        else:
            image_path = image_info  # if it's not a dict, use it directly
